function [location_pair,spectrum_of_targets]=cchengUFCLS(ImageCub, NumberOfClass)
...
%	Brief description:
%		The UFCLS is an unsupervised version of the FCLS developed by Heinz and Chang (2001).
%		Unlike FCLS, which requires complete a priori target knowledge to perform spectral unmixing
%		for abundance fraction estimation, UFCLS performs unsupervised target detection by using
%		FCLS to generate a set of potential targets directly from the data to be processed. It is identical
%		to UNCLS with the only difference that UFCLS uses FCLS instead of NCLS to find targets of
%		interest. So, its main goal is to find targets of interest without any prior knowledge. Because of
%		that, UFCLS is primarily used for target detection and endmember extraction as opposed to
%		FCLS, whose main functionality is to unmix data sample vectors in terms of abundance fractions.
%		Therefore, FCLS and UFCLS indeed have different applications.
